...
# Description: For parsing names - Last Middle First
...
def get_middle_name(row: pd.Series):
    row = [name for name in row if pd.notna(name)]  # Remove NA values
    return " ".join(row[1:]) if len(row) >= 2 else pd.NA
...
def process_name_parsing(df: pd.DataFrame):
    df_columns = [
        'id', 'Source', 'Owner', 'First Name', 'Middle Name', 'Last Name', 'ATTN', 'Address', 'City', 'State', 'Zip Code',
    ...
    df.loc[individual_mask, 'Last Name'] = df.loc[individual_mask, 'owner_copy'].str.split(" ").str[0].str.strip()
    ...
    owner_exploded_df.loc[tagging_mask, 'is_parsed'] = 'Y - Last First Middle'
